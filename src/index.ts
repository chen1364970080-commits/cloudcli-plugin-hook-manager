...
function render(
  ...
  // Wire add buttons
  for (const btn of Array.from(root.querySelectorAll<HTMLButtonElement>('[data-add-type]'))) {
    btn.addEventListener('click', () => {
      const type = btn.dataset.addType as 'PreToolUse' | 'PostToolUse';
  ...
  // Wire delete buttons
  for (const btn of Array.from(root.querySelectorAll<HTMLButtonElement>('[data-del-group]'))) {
    btn.addEventListener('click', () => {
      const type = btn.dataset.delType as 'PreToolUse' | 'PostToolUse';
  ...
  for (const toggle of Array.from(root.querySelectorAll<HTMLInputElement>('[data-toggle-group]'))) {
    toggle.addEventListener('change', () => {
      const type = toggle.dataset.toggleType as 'PreToolUse' | 'PostToolUse';
...
function renderSection(c: ThemeColors, data: HooksResponse, type: 'PreToolUse' | 'PostToolUse', label: string, desc: string): string {
...
async function saveData(root: HTMLElement, ctx: PluginContext, hooks: HooksConfig): Promise<void> {
  render(root, ctx, (root as any)._hmData ?? null, false, null, true);
  try {
    await api.rpc('PUT', 'hooks', { hooks });
    (root as any)._hmData = { hooks, settingsPath: ((root as any)._hmData as HooksResponse)?.settingsPath ?? '' };
    render(root, ctx, (root as any)._hmData, false, null, false);
  } catch (err) {
    render(root, ctx, (root as any)._hmData ?? null, false, (err as Error).message, false);
  }
}

function addHook(root: HTMLElement, ctx: PluginContext, type: 'PreToolUse' | 'PostToolUse', matcher: string, command: string): void {
  const data = (root as any)._hmData as HooksResponse | undefined;
  if (!data) return;

  const hooks = JSON.parse(JSON.stringify(data.hooks)) as HooksConfig;
  hooks[type].push({
    matcher,
...
function deleteGroup(root: HTMLElement, ctx: PluginContext, type: 'PreToolUse' | 'PostToolUse', idx: number): void {
  const data = (root as any)._hmData as HooksResponse | undefined;
  if (!data) return;
...
function toggleGroup(root: HTMLElement, ctx: PluginContext, type: 'PreToolUse' | 'PostToolUse', idx: number, enabled: boolean): void {
  const data = (root as any)._hmData as HooksResponse | undefined;
  if (!data) return;
